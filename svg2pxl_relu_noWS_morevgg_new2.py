...
from tensorflow.python.framework import ops as oooops
...
def my_GAN_G2(
    ...
    with tf.variable_scope("my_GAN_G", reuse = reuse):
        ...
        for i in range(lowest_resolution_log2, resolution_log2 + 1):
            ...
            n3 = (Conv2d(n2, 3, (3, 3), (1, 1), act = tf.nn.relu, padding = 'SAME', W_init = w_init, b_init = b_init, name = 'toRGB%d' % cur_resolutin))
            out = n3
            ...
            if idx > 0:
               ...
               last_rgb = (SubpixelConv2d(last_feature, name = 'sub_conv%d' % cur_resolutin))
...
def network_new2(
    ...
    #test_data, test_pad_nums = read_data(svg_dir, 5)    
    ...
    for temp_i in range(big_loop):
        ...
        for R in range(lowest_resolution_log2 + 4, target_resolution_log2 + 1):
        ...
        if not train_net or generate_pics:
            ...
            test_no = [13, 5]
            for j in range(2):
                data_pxl, pic_pad_nums = read_data(svg_dir + test_dir[j], num = test_no[j])
                ...
                for i in range(iters):
                    ...
                    tl.vis.save_images(out, [1, 1], test_output_dir + test_dir[j]+ "g_%d_output_%d.png" % (R, i))
                    out = sess.run(b_outputs[idx].outputs, {t_image_target : data_pxl[i]})
                    out = out.clip(0, 255)
                    out = np.array([clip_pic(out[0], pic_pad_nums[i])])
                    tl.vis.save_images(out, [1, 1], test_output_dir + test_dir[j]+ "b_%d_output_%d.png" % (R, i))
                    out = sess.run(t_image, {t_image_target : data_pxl[i]})
                    out = np.array([clip_pic(out[0], pic_pad_nums[i])])
                    tl.vis.save_images(out, [1, 1], test_output_dir + test_dir[j]+ "svg_%d_%d.png" % (R, i))
                    out = sess.run(t_image_target, {t_image_target : data_pxl[i]})
                    out = np.array([clip_pic(out[0], pic_pad_nums[i])])
                    tl.vis.save_images(out, [1, 1], test_output_dir + test_dir[j]+ "pxl_%d_%d.png" % (R, i))
...
if __name__ == "__main__":
        ...
        tl.global_flag['mode'] = 'my_gan'
        for i in range(1):
            network_new2(train_net = True, resume_network = False, use_L1_loss = False, big_loop = 1, use_vgg = True)
