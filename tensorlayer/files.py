...
def assign_params(sess, params, network):
    ...
    """
    ops = []
    print('read params:', len(params))
    print('net params:', len(network.all_params))
    for idx, param in enumerate(params):
        print(param.shape)
        print(network.all_params[idx].shape)
...
def load_and_assign_npz_dict(sess=None, name=None, network=None):
    """Load model from npz and assign to a network.
    ...
    if not os.path.exists(name):
        ...
        params = load_npz_dict(name=name)
        assign_params(sess, params, network)
        print("[*] Load {} SUCCESS!".format(name))
        return network
